...
@Component({
  selector: 'app-code-input-date',
  templateUrl: './code-input-date.component.html',
  styleUrls: ['./code-input-date.component.scss']
})
export class CodeInputDateComponent implements AfterViewInit {
  @ViewChild('codeBlock') codeBlock!: ElementRef;
  from: string = '';
  tabs: any[] = [];
  selectedTab: 'html' = 'html';
  ...
  constructor(
    ...
    this.codeService.getExample('code-input-date').subscribe(data => {
      const tabs = [
        { label: 'HTML', code: data.html, language: 'html', icon: data.icons?.html }
      ];
      this.tabs = [...tabs]; // Forzar referencia nueva para Angular
